	...
	do
		{
			...
			// Il faut qu'il ne soit pas deja testé, ni dans la tab de Atesté et il faut qu'il soit de bonne couleur
			if(courant.getX() >= 1 && jeu.getPion(courant.getX()-1,courant.getY()) == couleur && !Coord(courant.getX()-1, courant.getY()).presentDsTab(aTeste)
					&& !Coord(courant.getX()-1, courant.getY()).presentDsTab(dejaTeste) )
			{
				aTeste.push_back(Coord(courant.getX()-1,courant.getY()));
			}

			if(courant.getY() >= 1 && jeu.getPion(courant.getX(),courant.getY()-1) == couleur && !Coord(Coord(courant.getX(), courant.getY()-1)).presentDsTab(aTeste)
					&& !Coord(Coord(courant.getX(), courant.getY()-1)).presentDsTab(dejaTeste) )
			{
				aTeste.push_back(Coord(courant.getX(),courant.getY()-1));
			}
			if(courant.getX()+1 < jeu.getTaille() && jeu.getPion(courant.getX()+1,courant.getY()) == couleur && !Coord(courant.getX()+1, courant.getY()).presentDsTab(aTeste)
					&& !Coord(courant.getX()+1, courant.getY()).presentDsTab(dejaTeste) )
			{
				aTeste.push_back(Coord(courant.getX()+1,courant.getY()));
			}
			if(courant.getY()+1 < jeu.getTaille() && jeu.getPion(courant.getX(),courant.getY()+1) == couleur && !Coord(courant.getX(), courant.getY()+1).presentDsTab(aTeste)
			...
			// On rentre la coord courante dans DEJA TESTé
		...
		// On supprrime les pions et on retourne le nombre de pions supprimés
